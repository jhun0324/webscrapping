...
def openPage(url="https://www.webofknowledge.com", dirName='/abstracts'):
	# path to the directory to download files to
	download_path = os.getcwd() + dirName
...
def process_doi(doi):
	browser_pdf.get('http://doi.org/' + doi)
	url = browser_pdf.current_url
	downloaded = navigate_to_pdf(url)
	browser_pdf.close()
	return downloaded

def navigate_to_pdf(url):
	domain = urlparse(url)[1]
	if domain == 'www.sciencedirect.com':
		downloadpdfbutton = browser_pdf.find_element_by_partial_link_text("Download")
		downloadpdfbutton.click()
		getpdf = browser_pdf.find_element_by_link_text("Article")

		# cannot just grab url from html because Python requests library does not redirect correctly to the PDF
		getpdf.click()
		browser.switch_to.window(browser_pdf.window_handles[1])

		# sometimes takes a while to redirect to the actual pdf, so we wait until it does redirect
		while urlparse(browser_pdf.current_url)[1] == 'www.sciencedirect.com':
			time.sleep(1)
		pdfurl = browser_pdf.current_url
		browser_pdf.close()
		browser_pdf.switch_to.window(browser.window_handles[0])
		# WE HAVE SCIENCEDIRECT PDFS!!!
		return downloadpdf(pdfurl, title)
	elif domain == 'onlinelibrary.wiley.com':
		getpdf = browser_pdf.find_element_by_xpath("//li[@class='article-support__item--pdf ']/a")  # find pdf button
		...
		return downloadpdf(realpdflink, title)
		# WE HAVE WILEY ONLINE LIBRARY PDFS!!!
	elif domain == 'rd.springer.com':
		pdfbutton = browser_pdf.find_element_by_xpath("//a[@title='Download this article in PDF format']")
...
def setup(metaDataDirName = 'meta_data', pdfDirName = 'pdf',
		  metaDataDirSwitch = True, pdfDirSwitch = False):
...
# remove a abstract directory and make the same one again
...
query = 'TS="sustainable development"' + ' AND WC=(' + allCategories + ')'
...
browser_pdf = webdriver.Chrome(os.getcwd() + os.sep + 'chromedriver')
...
for filename in os.listdir(download_dir):
	...
	for i in range(numOfIndicator):
		...
		if isKeywordInArticle(articleKeywords, keywords) and hasDOI(article):
			...
			address = None
			downloaded = True
			df.loc[dataFrameIndex] = [doi, title, author, downloaded]
			dataFrameIndex += 1
